...
def fit_all_energies(
    ...
    *,
    ...
    fit_x = _fit_plane_global(
        df, s0, plane="x", n_bases=n_bases, degree=degree, lambda_reg=lambda_reg_xy, weight_col=weight_col
    )
    fit_y = _fit_plane_global(
        df, s0, plane="y", n_bases=n_bases, degree=degree, lambda_reg=lambda_reg_xy, weight_col=weight_col
    )
    ...
    L_prime = s_prime - s0
    x, xp, xxp = derived_params_at_sprime(Ax, Bx, Cx, Dx, L_prime)
    y, yp, yyp = derived_params_at_sprime(Ay, By, Cy, Dy, L_prime)
    out = pd.DataFrame(
        {
            "energy": energies,
            "x_a": Ax,
...
def derived_params_at_sprime(A, B, C, D, s_prime):
